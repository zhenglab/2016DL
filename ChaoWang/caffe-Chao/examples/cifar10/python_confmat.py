...
class PythonConfMat(caffe.Layer):
    ...
    def forward(self, bottom, top):
	    ...
	    average_accuracy = 0.0
		    ...
		    f.write(str(self.conf_matrix[i][j].astype(np.int))+'\t')
                sys.stdout.write('| %3.2f %%' % (self.conf_matrix[i][i]*100 / self.conf_matrix[i].sum()))
                average_accuracy += self.conf_matrix[i][i]*100 / self.conf_matrix[i].sum()
		f.write('%3.2f %%' % (self.conf_matrix[i][i]*100 / self.conf_matrix[i].sum()))		
                sys.stdout.write('\n')
		f.write('\n')
            sys.stdout.write('Number of test samples: %i \n' % self.conf_matrix.sum())
            
	    f.write('Number of test samples: %i \t\t' % self.conf_matrix.sum())
	    f.write('average percent: %3.2f %%' % float(average_accuracy / self.num_labels))
